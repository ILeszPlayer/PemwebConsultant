<x-app-layout>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-4 bg-[#F8F3FF]/40" id="chatContainer">
                    ...
                    @else
                        ...
                        <div class="flex justify-start animate-fadeIn">
                            <div class="max-w-[75%] bg-white border border-purple-100 text-gray-800 px-5 py-3 rounded-2xl rounded-tl-none shadow-sm text-sm leading-relaxed flex flex-col">
                                <span class="font-normal text-gray-700 msg-ai-text">{{ $msgText }}</span>
                                <div class="border-t border-purple-100/60 pt-2 mt-2">
                                    <span class="text-[10px] text-slate-400 italic font-normal tracking-wide">
                                        ⚠️ {{ $disclaimerText ?: 'Pesan otomatis sistem Hexa Space. Bukan merupakan diagnosis profesional psikologis. Gejala yang muncul akibat tekanan situasi yang sedang kamu alami.' }}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                @empty
                    <div class="text-center py-12 text-gray-400 text-sm italic">
                        Belum ada obrolan di sini. Ketik huruf depan kendala yang kamu rasakan untuk memulai...
                    </div>
                @endforelse
            </div>

            <div id="predictiveContainer" class="px-6 py-2 bg-white flex flex-wrap gap-2 transition-all duration-300 min-h-[0px]"></div>
                ...
                @if($session->status !== 'finished')
                    <form id="chatForm" action="{{ route('sessions.chat', $session->id) }}" method="POST" class="flex items-center space-x-3">
                        ...
                        <textarea
                            ...
                            name="message"
                            rows="1"
                            placeholder="Ketik keluhanmu (contoh: pusing, cemas, ukt, ibu, judol...)"
                            class="flex-1 border border-purple-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#C084FC] bg-[#F8F3FF]/30 text-gray-700 resize-none w-full"
                            required></textarea>
    ...
    <div id="moodModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 p-4">
        <div class="bg-white rounded-3xl max-w-md w-full p-6 text-center shadow-2xl border border-purple-100">
            ...
            <form action="{{ route('sessions.finish', $session->id) }}" method="POST" class="space-y-3">
                ...
                <button type="submit" name="final_mood" value="need_doctor" class="w-full py-3 px-4 border border-rose-200 hover:border-rose-400 bg-rose-50/40 rounded-2xl text-sm flex justify-between items-center text-rose-700 font-extrabold transition-all animate-pulse">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const masterPhrasesDataset = {
            ...
            'ibo': ['gimana cara aku ngomong ke ibu tentang ini?', 'tolong sarankan semua langkah agar aku bisa pulih', 'aku takut sekali mengecewakan ibu jika berkata sejujurnya'],
            ...
            messageInput.addEventListener('input', function () {
                ...
                if (contextScreenText.includes('judi') || contextScreenText.includes('gacor') || contextScreenText.includes('3 juta') || contextScreenText.includes('ukt')) {
                    if ('ibu'.startsWith(query) || query.includes('ib') || query.includes('ngomong')) {
                        matches = matches.concat(masterPhrasesDataset['ibo']);
                    }
